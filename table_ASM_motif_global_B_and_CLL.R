...
load(paste0(path,"annot/motif.rda"))

genes.motif=genes.coverage[countOverlaps(genes.coverage,motif.gr)>0]
genes.no.motif=genes.coverage[countOverlaps(genes.coverage,motif.gr)==0]
...
colnames(contingency_table.B)=c("No Motif","Motif")
...
contingency_table.B[2,2]=sum(unique(names(genes.motif))%in%unique(names(genes.asm.B)))
contingency_table.B[1,2]=sum(unique(names(genes.motif))%in%unique(names(genes.coverage.noasm.B)))
contingency_table.B[1,1]=sum(unique(names(genes.no.motif))%in%unique(names(genes.coverage.noasm.B)))
contingency_table.B[2,1]=sum(unique(names(genes.no.motif))%in%unique(names(genes.asm.B)))

colSums(contingency_table.B)
contingency_table.B[2,]/colSums(contingency_table.B)
...
df.ggplot.B<-data.frame(proportion=proportion.B,CI=CI.B,"Gene Type"=c("Motif","No Motif"),Tissue="B-cell")
...
contingency_table.CLL[2,2]=sum(unique(names(genes.motif))%in%unique(names(genes.asm.CLL)))
contingency_table.CLL[1,2]=sum(unique(names(genes.motif))%in%unique(names(genes.coverage.noasm.CLL)))
contingency_table.CLL[1,1]=sum(unique(names(genes.no.motif))%in%unique(names(genes.coverage.noasm.CLL)))
contingency_table.CLL[2,1]=sum(unique(names(genes.no.motif))%in%unique(names(genes.asm.CLL)))

colSums(contingency_table.CLL)
sum(colSums(contingency_table.CLL)
)
...
df.ggplot.CLL<-data.frame(proportion=proportion.CLL,CI=CI.CLL,"Gene Type"=c("Motif","No Motif"),Tissue="CLL")
