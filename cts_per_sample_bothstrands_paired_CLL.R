...
load(paste0(path,"annot/na1_CLL.rda")) # New NA files since changes in August 2022 (seems bismark methylation extractor was run on poorly sorted bams).
load(paste0(path,"annot/na2_CLL.rda")) # New NA files since changes in August 2022 (seems bismark methylation extractor was run on poorly sorted bams).
...
for (k in 1:length(samples)){
  ...
  files=files.all[substr(files.all,1,5)==samples[k]]
  grs=paste0(substr(files,1,9),".gr")
  dfs=paste0(substr(files,1,9),".df")
  ...
  for (i in 1:length(files)){
    ...
    print(i)
    load(paste0(path,"data/cov_to_small_CLL_qc/",files[i]))
    gr=get(grs[i])
    df=get(dfs[i])
  ...
  save(list=paste0("cts.",samples[k]),file=paste0(path,"data/cts_per_sample/bothstrands_paired_CLL/cts.",samples[k],".rda"))
}
